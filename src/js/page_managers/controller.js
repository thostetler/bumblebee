define([
  ...
  'underscore',
  'marionette',
  'js/page_managers/templates/results-page-layout',
  'js/page_managers/templates/results-control-row',
        ...
        function(widgetDom, widgetName) {
          ...
          var promise = app._getWidget(widgetName).done(function(widget) {
            if (
              ...
              self.persistentWidgets.indexOf(widgetName) > -1
            ) {
            ...
            if (widget) {
              var doRender = function() {
                ...
                if (
                  window.__PRERENDERED &&
                  widget.view &&
                  PRIORITY_WIDGETS.indexOf(widgetName) > -1 &&
                  $el.length > 0 &&
                  content.length > 0
                ) {
                  if (
                    typeof widget.view.handlePrerenderedContent === 'function'
                  ) {
              ...
              widget.assemble
                ? widget.assemble(app).done(doRender)
                : doRender();
      ...
      if (!pageName) {
        ...
        _.each(args, function(widgetName) {
          if (self.widgets[widgetName]) {
            ...
            var $wcontainer = self.view.$el.find(
            ...
            if ($wcontainer.length) {
              ...
              try {
                ...
                console.error(
                  'Error when displaying widget: ' + widgetName,
                  e.message,
                  e.stack
                );
              }
  ...
  _.extend(
    PageManagerController.prototype,
    PageManagerViewMixin,
    Dependon.BeeHive,
    {
