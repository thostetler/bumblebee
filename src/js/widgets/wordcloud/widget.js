define([
  ...
  'js/widgets/wordcloud/templates/wordcloud-template',
  'js/widgets/wordcloud/templates/selected-list-template',
  ...
  var WordCloudView = Marionette.ItemView.extend({
    ...
    draw: function() {
      ...
      if (!this.$('#words-group').length) {
        var g = svg
          .attr('aria-label', 'wordcloud chart')
          .append('g')
          .attr('id', 'words-group')
          .attr('width', width)
          .attr('height', height)
  ...
  var WordCloudModel = Backbone.Model.extend({
    ...
    buildWCDict: function() {
      ...
      meanTF =
        _.reduce(
          _.map(_.values(dict), function(x) {
      ...
      renderVals.fill
        .domain([0, 0.25, 0.5, 0.75, 1].map(renderVals.fill.invert))
        .range(this.get('colorRange'))
        .clamp(true);

      var pixelScale = d3.scale
        .log()
        .domain([min, max])
        .range([30, 70]);
