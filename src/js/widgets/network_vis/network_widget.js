import d3 from 'd3';
import authorDetailsTemplate from 'js/widgets/network_vis/templates/author-details-template.html';
import containerTemplate from 'js/widgets/network_vis/templates/container-template.html';
import DefaultDetailsTemplate from 'js/widgets/network_vis/templates/default-details-template.html';
import filterContainerTemplate from 'js/widgets/network_vis/templates/filter-container-template.html';
import graphContainerTemplate from 'js/widgets/network_vis/templates/graph-container-template.html';
import groupDetailsTemplate from 'js/widgets/network_vis/templates/group-details-template.html';
import loadingTemplate from 'js/widgets/network_vis/templates/loading-template.html';
import metadataTemplate from 'js/widgets/network_vis/templates/network_metadata.html';
import notEnoughDataTemplate from 'js/widgets/network_vis/templates/not-enough-data-template.html';
  ...
  onRender: function() {
    ...
    if (oneYear) {
      ...
      this.$('.time-series-container').html(
        'Not enough data to show a graph: All papers are from the year ' + singleYear
      );
      return;
    }
  ...
  showDetailView: function(selected) {
    ...
    if (!d.children) {
      ...
      allPapers = _.sortBy(allPapers, function(list) {
        ...
        return (
          (((((list[1] - numAuthors[0]) / (numAuthors[numAuthors.length - 1] - numAuthors[0])) *
            (list[1] / graphData.bibcode_dict[list[0]].authors.length - percentAuthors[0])) /
            (percentAuthors[percentAuthors.length - 1] - percentAuthors[0])) *
            (graphData.bibcode_dict[list[0]].citation_count / graphData.bibcode_dict[list[0]].authors.length -
              numCitations[0])) /
...
var GraphView = Marionette.ItemView.extend({
  ...
  customizeScales: function(graphData) {
    ...
    this.scales.citationLimit =
      citation_counts[this.config.numberOfLabelsToShow] || citation_counts[citation_counts.length - 1];
    ...
    if (this.model.get('widgetName') === 'AuthorNetwork') {
      ...
      root.children.forEach((group) => {
        ...
        group.children.forEach((author) => {
          output += `${groupName},"${author.name}","${author.papers.join(',')}",${author.citation_count},${
            author.read_count
          }\n`;
        });
      });
