import d3 from 'd3';
import CitationsTableTemplate from 'js/widgets/metrics/templates/citations_table.html';
import GraphTemplate from 'js/widgets/metrics/templates/graph_template.html';
import IndicesTableTemplate from 'js/widgets/metrics/templates/indices_table.html';
import MetricsContainer from 'js/widgets/metrics/templates/metrics_container.html';
import PaperTableTemplate from 'js/widgets/metrics/templates/paper_table.html';
import ReadsTableTemplate from 'js/widgets/metrics/templates/reads_table.html';
...
var GraphView = Marionette.ItemView.extend({
  ...
  drawLineGraph: function() {
    ...
    (function() {
      ...
      var createInfoBoxHtml = _.memoize(
        function(data) {
          if (!data) {
            return;
          }
...
var MetricsWidget = BaseWidget.extend({
  ...
  createTableData: function(response) {
    ...
    data.readsModelData = {
      ...
      medianNumberOfReads: [
        generalData.total['median number of reads'],
        generalData.refereed['median number of reads'],
      ],
  ...
  createGraphViews: function(response) {
    ...
    this.childViews.papersGraphView.model.set(
      'graphData',
      this.dataExtractor.plot_paperhist({
        norm: false,
    ...
    this.childViews.citationsGraphView.model.set(
      'graphData',
      this.dataExtractor.plot_citshist({
        norm: false,
    ...
    this.childViews.readsGraphView.model.set(
      'graphData',
      this.dataExtractor.plot_readshist({
        norm: false,
    ...
    if (response['time series']) {
      this.childViews.indicesGraphView.model.set(
        'graphData',
        this.dataExtractor.plot_series({
    ...
    var sendRepeatingRequest = function(start) {
      ...
      this._executeSearch(q).done(function(request) {
        allBibcodes = allBibcodes.concat(
          request.get('response.docs').map(function(o) {
            return o.bibcode;
          })
        );
