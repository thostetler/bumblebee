define([
  ...
  'js/wraps/templates/paper-network-data',
  'js/wraps/templates/paper-network-container',
  'js/widgets/network_vis/templates/not-enough-data-template',
  'js/wraps/templates/paper-network-link-data',
  ...
  options.graphView = Marionette.ItemView.extend({
    ...
    events: {
      ...
      'click .filter-remove': function(e) {
        ...
        var displayName = _.findWhere(
          this.model.get('graphData').summaryGraph.nodes,
          { id: id }
        ).node_name;
    ...
    getConfig: function() {
      ...
      this.config.radius =
        Math.min(this.config.width, this.config.height) / 1.5;
      this.config.outerRadius =
        Math.min(this.config.width, this.config.height) * 0.33;
      this.config.innerRadius =
        Math.min(this.config.width, this.config.height) * 0.21;
    },
    ...
    generateCachedVals: function() {
      ...
      this.cachedVals.line = d3.svg.line
        .radial()
        .interpolate('bundle')
        .tension(0.3)
        .radius(function(d) {
      ...
      this.cachedVals.svg = d3
        .select(this.$('svg.network-chart')[0])
        ...
        .attr(
          ...
          'translate(' +
            that.config.width / 2 +
            ',' +
            that.config.height / 2 +
            ')'
        );

      this.cachedVals.arc = d3.svg
        .arc()
        .innerRadius(that.config.innerRadius)
        .outerRadius(that.config.outerRadius);

      this.cachedVals.pie = d3.layout
        .pie()
        .value(function(d) {
    ...
    computeScales: function() {
      ...
      this.scales.tensionScale = d3.scale
        .linear()
      ...
      this.scales.initialFontScale = d3.scale
        .linear()
        .domain([d3.min(paperCounts), d3.max(paperCounts)])
        .range([9, 14]);

      this.scales.fill = d3.scale
        .ordinal()
        .domain([1, 2, 3, 4, 5, 6])
        .range([
          'hsl(282, 59%, 61%)',
    ...
    renderGraph: function() {
      ...
      text
        .on('mouseover', fadeText('mouseenter'))
        ...
        return function(g, i, j) {
          ...
          if (event === 'mouseenter') {
            if (g.data.node_name > 7) {
              d3.select(this).attr(
                'fill',
                d3.hsl(that.config.noGroupColor).darker(0.7)
              );
            } else {
              d3.select(this).attr(
                'fill',
                d3.hsl(that.scales.fill(g.data.node_name)).darker(0.7)
              );
            }
      ...
      function fadeText(event) {
        return function(g, j, i) {
          // properly color the group background
          var nodeName = d3.select(this.parentNode).data()[0].data.data
            .node_name;
          ...
          if (event === 'mouseenter') {
            if (nodeName > 7) {
              ...
              d3.select(pieNode).attr(
                'fill',
                d3.hsl(that.scales.fill(nodeName)).darker(0.7)
              );
            }
          } else if (nodeName > 7) {
    ...
    showSelectedEntity: function() {
      ...
      // it's a node id
        ...
        var groupData = _.findWhere(
          that.model.get('graphData').summaryGraph.nodes,
          function(g) {
            return g.id == entity;
          }
        );
        ...
        summaryData.processedTopCommonReferences = _.sortBy(
          summaryData.processedTopCommonReferences,
          function(n) {
            return n[1];
          }
        ...
        data.inFilter = !!Marionette.getOption(that, 'filterCollection').get(
          groupData.node_name
        );
        data.topNodes = topNodes.slice(0, 30);
        data.groupIdToShow = groupData.node_name;
        data.borderColor =
          groupData.node_name < 8
            ? that.scales.fill(groupData.node_name)
            : that.config.noGroupColor;
    ...
    changeMode: function() {
      ...
      function arcTween(d, i) {
        var i = d3.interpolate(
          { startAngle: this.oldStartAngle, endAngle: this.oldEndAngle },
          d
        );
        return function(t) {
          var b = i(t);
      ...
      groups = svg
        ...
        .attr('transform', function(d) {
          return (
            ...
            ')' +
            'translate(' +
            that.config.outerRadius +
            ',0)'
          );
        });

      svg
        .selectAll('.summary-label-container')
    ...
    getAllLinks: function(id) {
      ...
      _.each(fullGraph.links, function(l, i) {
        if (
          indexes.indexOf(l.source) !== -1 ||
          indexes.indexOf(l.target) !== -1
        ) {
          links.push(l);
        }
      });
