...
handlers[ApiFeedback.CODES.MAKE_SPACE] = function() {
  this.getBeeHive()
    ...
    .once(
      ...
      _.bind(function(apiRequest, psk) {
...
handlers[ApiFeedback.CODES.SEARCH_CYCLE_STARTED] = function(feedback) {
  ...
  if (
    feedback.request &&
    feedback.request.get('target').indexOf('search') > -1 &&
    feedback.query &&
    !feedback.numFound
  ) {
...
handlers[ApiFeedback.CODES.SEARCH_CYCLE_FAILED_TO_START] = function(feedback) {
      ...
      case 429:
        alerts.alert(
          new ApiFeedback({
            code: ApiFeedback.CODES.ALERT,
            msg:
              'We are sorry, it looks like you have exhausted allowed quota for: ' +
              target +
              '. The limits will be reset in (usually) in 24 hours. If you believe this is an error or if you need to do something special, please contact us using the feedback button and describe what you are trying to accomplish. We may be able to help or provide efficient way of searching through ADS database. We also have an API that can be useful!',
            modal: true,
          })
        );
        return;
      case 405: // method not allowed
        ...
        alerts.alert(
          new ApiFeedback({
            code: ApiFeedback.CODES.ALERT,
            msg:
              'I am afraid you have just discovered a glitch in our system. (Congratulations!). What you are trying to do should be possible. It is likely a mis-configuration. Please try to reload the webpage; if problem persists - then send us an angry email and wait for a day.',
            modal: true,
          })
        );
        return;
          ...
          .fail(function() {
            ...
            alerts.alert(
              new ApiFeedback({
                ...
                msg:
                  'The security token used to access our API has expired and we are unable to refresh it (though we tried). You will have to reload the webpage. Sorry about that!',
                modal: true,
              })
            );
          });
  ...
  analytics(
    ...
    'event',
    'error',
    'unrecoverable-' + xhr.status,
    'request=' + apiRequest.url() + ' token=...' + getAccessTokenStump
  );

  var msg;
  var msgArr = [
    'We are having trouble accessing: ' +
      target +
      '. The ADS backend service is probably down, or else we’ve been struck by an asteroid.',
    'We are having trouble accessing: ' +
      target +
      '. A solar flare must have taken down our servers! Or else the ADS backend service is down momentarily.',
    'Was that a supernova or are our servers just down? We are having trouble accessing: ' +
      target +
      ' but we should be back in a flash.',
    'The alien mothership has arrived! Or else the ADS backend service is down momentarily. We are having trouble accessing: ' +
      target +
    ...
    'Universal Translator Malfunction: Unable to Communicate with ADS Droids. (The ADS backend service is down momentarily and we are having trouble accessing: ' +
      target +
      ' but we should be back up soon.',
    'UV catastrophe detected! (The ADS backend service is down momentarily and we are having trouble accessing: ' +
      target +
      ' but we should be back up soon.)',
    'The ADS Droids got distracted by the Missing Mass Problem instead of working on: ' +
      target +
      '. Be back in a minute.',
  ];
  ...
  switch (xhr.status) {
    ...
    case 408: // proxy timeout
      msg =
        'This particular query is taking too long. Please wait a moment and retry. If the problem persits, our backend is likely overwhelmed and will be sluggish for a little longer.';
      break;
    ...
    default:
      msg =
        "Today, you are doubly unlucky. Not only are you experiencing an 'unrecoverable' error, but you have to read this default error message. Sorry about that! We have been notified about the problem.";
      break;
  }
    ...
    if (/orcid/.exec(target) && feedback.error.status == 401) {
      alerter.alert(
        new ApiFeedback({
          code: ApiFeedback.CODES.ALERT,
          msg:
            'Your connection with Orcid is no longer valid, it may be that you revoked your token. To continue using this feature, please log back in to Orcid.',
          modal: true,
        })
      );
  ...
  var mediator = new (FeedbackMediator.extend({
    ...
    onAppStarting: function() {
      ...
      this.getPubSub().subscribe(
        pubsub.START_SEARCH,
        _.bind(function(apiQuery, senderKey) {
          var pubsub = this.getPubSub();
          var app = this.getApp();
          var qm;
          var widget;
          ...
          if (this.getCurrentPage() !== 'SearchPage' && app.getWidgetRefCount('Results') <= 0) {
            ...
            app
              ...
              .navigate('results-page', { replace: false })
              .then(function() {
