...
import { Alert, Button, ControlLabel, Form, FormControl, FormGroup, HelpBlock } from 'react-bootstrap';
...
const loginStatusMessage = ({ status, error }) => {
  switch (status) {
    ...
    case 'failure':
      return <span className="text-danger">{error ? error : 'Login failed, try changing the mirror site.'}</span>;
    case 'success':
