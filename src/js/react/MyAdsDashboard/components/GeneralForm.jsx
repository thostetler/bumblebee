...
import { Checkbox, ControlLabel, Form, FormControl, FormGroup, Radio } from 'react-bootstrap';
...
const getStatusMessage = ({ status, error, editing, noSuccess }) => {
  switch (status) {
    ...
    case 'success':
      return <span className="text-success">{noSuccess ? '' : `Notification ${editing ? 'saved' : 'created'}!`}</span>;
    default:
      return null;
  }
};
          ...
          >
            <div>
              ...
              <span className="text-primary">
                <i className="fa fa-bullhorn" aria-hidden="true"></i> Check out the Gif on the right for an example.{' '}
            ...
            <div>
              <img src="styles/img/myADS.gif" alt="how to edit query on results page" style={{ maxWidth: '800px' }} />
            </div>
          </div>
        )}
      </div>
