define([
  ...
  var mockDocs = {
    ...
    links_data: [
      '{"access": "open", "instances": "", "title": "", "type": "preprint", "url": "http://arxiv.org/abs/1604.00988"}',
      '{"access": "", "instances": "1", "title": "", "type": "ned", "url": "https://ned.ipac.caltech.edu/cgi-bin/objsearch?search_type=Search&refcode=2017MNRAS.467.4015H"}',
      '{"access": "open", "instances": "", "title": "", "type": "electr", "url": "https://doi.org/10.1093%2Fmnras%2Fstx175"}',
      '{"access": "", "instances": "74", "title": "", "type": "simbad", "url": "http://$SIMBAD$/simbo.pl?bibcode=2017MNRAS.467.4015H"}',
      '{"access": "open", "instances": "", "title": "", "type": "pdf", "url": "https://academic.oup.com/mnras/pdf-lookup/doi/10.1093/mnras/stx175"}',
      '{"access": "", "instances": "", "title": "European HST References (EHST)", "type": "data", "url": "http://archives.esac.esa.int/ehst/#bibcode=2017MNRAS.467.4015H"}',
      '{"access": "", "instances": "", "title": "MAST References (HST)", "type": "data", "url": "https://archive.stsci.edu/mastbibref.php?bibcode=2017MNRAS.467.4015H"}',
    ...
    esources: ['EPRINT_HTML', 'EPRINT_PDF', 'PUB_HTML', 'PUB_PDF'],
    issue: '4',
    pub: 'Monthly Notices of the Royal Astronomical Society',
    volume: '467',
    data: ['ESA:1', 'IRSA:1', 'MAST:1', 'NED:1', 'SIMBAD:74'],
    issn: ['0035-8711'],
    ...
    pub_raw:
      'Monthly Notices of the Royal Astronomical Society, Volume 467, Issue 4, p.4015-4035',
    title: [
    ...
    '[citations]': { num_references: 98, num_citations: 34 },
  };
  ...
  var mockFullTextSources = [
    {
      ...
      url:
        'http://adsabs.harvard.edu/cgi-bin/nph-data_query?bibcode=2017MNRAS.467.4015H&link_type=EJOURNAL',
    },
    {
      open: false,
      name: 'Publisher PDF',
      url:
        'http://adsabs.harvard.edu/cgi-bin/nph-data_query?bibcode=2017MNRAS.467.4015H&link_type=ARTICLE',
    },
    {
      open: true,
      name: 'arXiv e-print',
      url:
        'http://adsabs.harvard.edu/cgi-bin/nph-data_query?bibcode=2017MNRAS.467.4015H&link_type=PREPRINT',
    },
  ];
  ...
  describe('Meta Tags Widget', function() {
    ...
    it('can get docs from cache (if available)', function() {
      ...
      const cachedDoc = widget.getCachedDoc(mockDocs.bibcode);
      const actual = JSON.stringify(cachedDoc, Object.keys(cachedDoc).sort());
      const mockDoc = { issn: undefined, isbn: undefined, ...mockDocs };
      const expected = JSON.stringify(mockDoc, Object.keys(mockDoc).sort());

      expect(actual).to.equal(expected);
      minsub.beehive.removeObject('DocStashController');
      minsub.beehive.addObject(
        'DocStashController',
    ...
    it('updates the page meta tags', function() {
      ...
      var metas = $('meta[data-highwire="true"]')
        .map(function() {
