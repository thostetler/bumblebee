    ...
    describe('Author Affiliation Tool (author_affiliation_tool.spec.js)', function () {
      ...
      describe('form actions', function () {
        ...
        it('updates year and starts new search when changed', function () {
          ...
          yearSelect.simulate('change', { target: {
            value: year
          }});

          expect(state().year).to.eql(year, 'state should have the new year');
          expect(this.w.fetchAffiliationData.calledOnce).to.eql(true, 'fetch data was called');
          expect(this.w.fetchAffiliationData.args[0][0]).to.eql(state().ids, 'fetch data was called with ids');
          expect(this.w.fetchAffiliationData.args[0][1]).to.eql(year, 'fetch data was called with the new offset');
        });
        ...
        it('updates author and starts new search when changed', function () {
          ...
          authorSelect.simulate('change', { target: {
            value: maxAuthor
          }});

          expect(state().author).to.eql(maxAuthor, 'state should have the new author count');
          expect(this.w.fetchAffiliationData.calledOnce).to.eql(true, 'fetch data was called');
          expect(this.w.fetchAffiliationData.args[0][0]).to.eql(state().ids, 'fetch data was called with ids');
          expect(this.w.fetchAffiliationData.args[0][2]).to.eql(maxAuthor, 'fetch data was called with the new author count');
        });
      });
