module.exports = function (grunt) {
  ...
  grunt.registerMultiTask('build-it', 'sldkjfs', function () {
    var paths = [
      [
        "discovery.config",
        "./discovery.config.js"
    ...
    grunt.registerTask('compile', function () {
      ...
      var stream = fs.createWriteStream(path.resolve('build', 'app.built.js'));
      stream.on('finish', done);
      requirejs.tools.useLib(function(require) {
        require(['transform'], function (transform) {

          Object.keys(pathsByIds).forEach(function(filePath) {
            var id = pathsByIds[filePath];
            var fullPath = path.resolve(filePath);
            var contents = fs.readFileSync(fullPath, 'utf8');
            var transformed = transform.toTransport('', id, fullPath, contents);
            stream.write(transformed, function () {
              console.log(fullPath, 'added to build');
            });
          });
          stream.end();
        });
