...
(function (factory) {
  ...
  if (typeof define === 'function' && define.amd) define(['underscore', 'backbone', 'exports'], factory); // Next for Node.js or CommonJS.
  else if ((typeof exports === "undefined" ? "undefined" : _typeof(exports)) === 'object') factory(require('underscore'), require('backbone'), exports); // Finally, as a browser global.
  ...
  Stickit.ViewMixin = {
    ...
    unstickit: function unstickit(model, bindingSelector) {
      ...
      _.invoke(_.uniq(models), 'trigger', 'stickit:unstuck', this.cid); // Call `_destroy` on a unique list of the binding callbacks.
    ...
    stickit: function stickit(optionalModel, optionalBindingsConfig) {
      ...
      this._modelBindings || (this._modelBindings = []); // Add bindings in bulk using `addBinding`.

      this.addBinding(model, bindings); // Wrap `view.remove` to unbind stickit model and dom events.
    ...
    addBinding: function addBinding(optionalModel, selector, binding) {
      ...
      binding = binding || {}; // Support jQuery-style {key: val} event maps.
      ...
      var $el = selector === ':el' ? this.$el : this.$(selector); // Clear any previous matching bindings.

      this.unstickit(model, selector); // Fail fast if the selector didn't match an element.

      if (!$el.length) return; // Allow shorthand setting of model attributes - `'selector':'observe'`.

      if (_.isString(binding)) binding = {
        observe: binding
      }; // Handle case where `observe` is in the form of a function.

      if (_.isFunction(binding.observe)) binding.observe = binding.observe.call(this); // Find all matching Stickit handlers that could apply to this element
      // and store in a config object.

      var config = getConfiguration($el, binding); // The attribute we're observing in our config.

      var modelAttr = config.observe; // Store needed properties for later.

      config.selector = selector;
      config.view = this; // Create the model set options with a unique `bindId` so that we
      // can avoid double-binding in the `change:attribute` event handler.

      var bindId = config.bindId = _.uniqueId(); // Add a reference to the view for handlers of stickitChange events


      var options = _.extend({
        stickitChange: config
      }, config.setOptions); // Add a `_destroy` callback to the configuration, in case `destroy`
      ...
      if (modelAttr) {
        ...
        _.each(config.events, function (type) {
          ...
          var listener = function listener(event) {
            var val = applyViewFn.call(this, config.getVal, $el, event, config, slice.call(arguments, 1)); // Don't update the model if false is returned from the `updateModel` configuration.
        ...
        }, this); // Setup a `change:modelAttr` observer to keep the view element in sync.
  ...
  _.extend(Backbone.View.prototype, Stickit.ViewMixin); // Helpers
  // -------


  var slice = [].slice; // Evaluates the given `path` (in object/dot-notation) relative to the given
  ...
  var setAttr = function setAttr(model, attr, val, options, config) {
    ...
    if (config.set) {
      ...
      value[attr] = val; // If `observe` is defined as an array and `onSet` returned
  ...
  var getConfiguration = Stickit.getConfiguration = function ($el, binding) {
    ...
    handlers.push(binding); // Merge handlers into a single config object. Last props in wins.

    var config = _.extend.apply(_, handlers); // `updateView` is defaulted to false for configutrations with
    ...
    _.each(config.attributes || [], function (attrConfig) {
      ...
      var updateAttr = function updateAttr() {
        var updateType = _.contains(props, attrConfig.name) ? 'prop' : 'attr',
            val = getAttr(model, observed, attrConfig); // If it is a class then we need to remove the last value and add the new.
  ...
  var initializeVisible = function initializeVisible($el, config, model, modelAttr) {
    ...
    var visibleCb = function visibleCb() {
      var visible = config.visible,
          ...
          isVisible = !!val; // If `visible` is a function then it should return a boolean result to show/hide.
  ...
  Stickit.addHandler([{
    ...
    getVal: function getVal($el) {
      ...
      if ($el.length > 1) {
        ...
        val = $el.prop('checked'); // If the checkbox has a value attribute defined, then
    ...
    update: function update($el, val, model, options) {
      var optList,
          ...
          isMultiple = $el.prop('multiple'); // If there are no `selectOptions` then we assume that the `<select>`
      ...
      var addSelectOptions = function addSelectOptions(optList, $el, fieldVal) {
        _.each(optList, function (obj) {
          ...
          var fillOption = function fillOption(text, val, disabled) {
            option.text(text);
            optionVal = val; // Save the option value as data so that we can reference it later.
          ...
          fillOption(text, val, disabled); // Determine if this option is selected.
      ...
      $el.find('*').remove(); // The `list` configuration is a function that returns the options list or a string
      ...
      if (optList instanceof Backbone.Collection) {
        ...
        collection.once('stickit:selectRefresh', removeCollectionListeners, this); // Listen to the collection and trigger an update of the select options

        collection.on('add remove reset sort', refreshSelectOptions, this); // Remove the previous model event listener
