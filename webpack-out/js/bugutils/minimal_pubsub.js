 ...
 *
 */
define(['underscore', 'backbone', 'js/components/query_mediator', 'js/services/pubsub', 'js/components/beehive', 'js/components/pubsub_events', 'js/components/api_query', 'js/components/api_request', 'js/components/api_feedback', 'js/components/api_response'], function (_, BackBone, QueryMediator, PubSub, BeeHive, PubSubEvents, ApiQuery, ApiRequest, ApiFeedback, ApiResponse) {
  var MinimalPubsub = function MinimalPubsub() {
    this.beehive = null;
    this.pubsub = null;
  ...
  _.extend(MinimalPubsub.prototype, Backbone.Events, PubSubEvents, {
    initialize: function initialize(options) {
      ...
      this.beehive.addService('Api', options.Api || {
        request: function request(req, context) {
      ...
      this.beehive.activate(this.beehive); // add Query-mediator; it is using an app object, so we'll feed it
      // a fake one
    ...
    subscribe: function subscribe(signal, callback) {
      var args = _.toArray(arguments);

      args.unshift(this.key);
      this.pubsub.subscribe.apply(this.pubsub, args);
    },
    subscribeOnce: function subscribeOnce(signal, callback) {
      var args = _.toArray(arguments);
    ...
    request: function request(apiRequest, params) {
      if (this.verbose) {
    ...
    createQuery: function createQuery(data) {
