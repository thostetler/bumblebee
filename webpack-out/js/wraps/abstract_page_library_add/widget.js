define(['js/widgets/query_info/query_info_widget', 'js/widgets/base/base_widget', 'js/wraps/abstract_page_library_add/template.html'], function (QueryInfoWidget, BaseWidget, template) {
  var AbstractLibraryWidget = QueryInfoWidget.extend({
    ...
    activate: function activate(beehive) {
      ...
      pubsub.subscribe(pubsub.DISPLAY_DOCUMENTS, this.onDisplayDocuments); // check if user is signed in (because widget was just instantiated, but app might have been running for a while
    ...
    onDisplayDocuments: function onDisplayDocuments(apiQuery) {
      var id = this.parseIdentifierFromQuery(apiQuery);
