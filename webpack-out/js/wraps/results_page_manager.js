define(['js/page_managers/controller', 'js/page_managers/three_column_view', 'js/page_managers/templates/results-page-layout.html', 'js/mixins/side_bar_manager', 'jquery', 'utils'], function (PageManagerController, PageManagerView, PageManagerTemplate, SideBarManagerMixin, $, utils) {
  var PageManager = PageManagerController.extend({
    ...
    adjustStickyElements: function adjustStickyElements() {
      ...
      if (window.matchMedia('(max-width: 788px)').matches && $('#search-results-sort').get(0) && $('#search-results-sort').get(0).getBoundingClientRect().bottom <= 0) {
    ...
    stickElements: function stickElements(top) {
      // button container
      $('#search-results-actions').addClass('sticky'); // add to library widget

      $('#query-info-container').attr('style', "position:fixed;top:".concat(top, "px;right:0"));
    },
