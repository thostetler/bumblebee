define(['js/components/api_response', 'js/components/api_request', 'js/components/api_query', 'js/components/solr_response', 'js/widgets/facet/factory', 'js/components/api_targets', 'analytics', 'cache', 'underscore', 'js/widgets/facet/reducers'], function (ApiResponse, ApiRequest, ApiQuery, SolrResponse, FacetFactory, ApiTargets, analytics, Cache, _, Reducers) {
  return function () {
    ...
    widget.translateSimbid = function (apiResponse, id) {
      var that = this;
      var simbids = apiResponse.toJSON().facet_counts.facet_fields.simbad_object_facet_hier.map(function (id, i) {
      ...
      function done(data) {
        ...
        enhancedResponse.facet_counts.facet_fields.simbad_object_facet_hier = enhancedResponse.facet_counts.facet_fields.simbad_object_facet_hier.map(function (facet, i) {
          if (i % 2 == 0) {
            var facetParts = facet.split('/');
            var simbid = facetParts[facetParts.length - 1]; // in the form "1/Star/* bet Pic"

            var facetVal = facetParts.slice(0, 2).concat([data[simbid].canonical]).join('/'); // store it in case the widget is submitted later
    ...
    widget.submitFilter = function (operator) {
      ...
      var conditions = selectedFacets.map(function (c) {
        ...
        if (this._simbidCache[c]) {
          var facetName = c.split('/').slice(0, 2).concat([this._simbidCache[c]]).join('/');
        } else {
          var facetName = c;
        }
