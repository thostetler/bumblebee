define(['js/widgets/metrics/widget', 'js/components/api_feedback'], function (MetricsWidget, ApiFeedback) {
  var Widget = MetricsWidget.extend({
    initialize: function initialize(options) {
      ...
      var handlePMMessages = function handlePMMessages(event, data) {
        if (event === 'broadcast-payload') {
          this.payload = data; // if these happen out of order, then broadcast payload now
    ...
    ingestBroadcastedPayload: function ingestBroadcastedPayload(data) {
      ...
      this.getMetrics([this._bibcode]).done(function () {
        self._closed = false; // Everything worked, show the widget
        ...
        self._closed = true; // if the metrics fail, kill it
