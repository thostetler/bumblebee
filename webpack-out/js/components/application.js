 ...
 *
 */
define(['underscore', 'jquery', 'backbone', 'module', 'js/components/beehive', 'js/mixins/api_access'], function (_, $, Backbone, module, BeeHive, ApiAccess) {
  ...
  _.extend(Application.prototype, {
    ...
    loadModules: function loadModules(config, options) {
      ...
      promises.length === 1 && promises.push(promise); // add a handler for updating the app loading bar
    ...
    _loadModules: function _loadModules(sectionName, modulePrescription, ignoreErrors, lazyLoad) {
      ...
      var callback = function callback() {
        ...
        try {
           ...
           */
          var pubsub = self.getService('PubSub').getHardenedInstance();
