 ...
 *
 */
define(['underscore', 'js/components/generic_module', 'js/components/api_query_updater'], function (_, GenericModule, ApiQueryUpdater) {
  var TreeNode = function TreeNode(operator, value) {
  ...
  var RuleNode = function RuleNode() {
    this.validOperators = {
      AND: 'AND',
      OR: 'OR',
      DEFOP: 'DEFOP'
  ...
  _.extend(RuleNode.prototype, {
    ...
    _serializeValue: function _serializeValue() {
    ...
    _modifyOperator: function _modifyOperator(ret) {
      if (this.modifier) {
        if (this.modifier == '-') {
          ret.operator += '_not';
        } else if (this.modifier == '=') {
          ret.operator += '_exactly';
        }
      }
    ...
    setValue: function setValue(val, type) {
      this.value = val;
    ...
    setModifier: function setModifier(m) {
      this.modifier = m;
    },
    getModifier: function getModifier() {
      return this.modifier;
    },
    setFuzzy: function setFuzzy(f) {
      this.fuzzy = f.replace('~', '');
    },
    setBoost: function setBoost(v) {
      this.boost = v.replace('^', '');
    },
    setOperator: function setOperator(o) {
      this.operator = o;
    },
    addChild: function addChild(ruleNode) {
      this.rules = this.rules || [];
  ...
  var RulesTranslator = GenericModule.extend({
     ...
     * @param qtree
     */
    convertQTreeToRules: function convertQTreeToRules(qtree) {
     ...
     * @private
     */
    _parentize: function _parentize(qtree, parent) {
      var self = this;
      qtree.parent = parent;
    ...
    _extractRules: function _extractRules(qtree) {
    ...
    _extractRule: function _extractRule(qtree, ruleNode) {
      // ruleNode can be null
      var ruleNode;
      var inputNode;
      var self = this; // console.log('extracting', qtree.name, ruleNode);
     ...
     * @returns String
     */
    buildQuery: function buildQuery(rules) {
      if (rules.rules) {
    ...
    _buildQueryTree: function _buildQueryTree(treeNode, rules) {
      var self = this;
    ...
    _buildOneRule: function _buildOneRule(rule) {
      var val;
      var q;
      var field;
    ...
    _cleanupFieldValue: function _cleanupFieldValue(field, value) {
      value = value.trim();
    ...
    extractFunctionValues: function extractFunctionValues(funcname, qtree) {
      ...
      _.each(qtree.children, function (child) {
        ...
        this._extractRule(child, childNode); // detect more complicated case of the nested queries and bail
    ...
    _getOriginalQuery: function _getOriginalQuery(qtree) {
      var t = qtree;
    ...
    addValidFunctions: function addValidFunctions(fs) {
      this.validFunctions = _.extend(this.validFunctions, fs);
    },
    setValidFunctions: function setValidFunctions(vals) {
      this.validFunctions = vals;
    },
    getValidFunctions: function getValidFunctions() {
