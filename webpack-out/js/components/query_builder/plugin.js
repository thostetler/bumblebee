    ...
    initialize: function initialize(options) {
      ...
      this.$el.queryBuilder({
        ...
        filters: [{
          ...
          input: function input($rule, filter) {
          ...
          onAfterSetValue: function onAfterSetValue($rule, value, filter, operator) {
        ...
        extend: {
          createFunctionInputs: function createFunctionInputs(profiles) {
            var $target;
            var values;
            var $current;
            var $container;
          ...
          getGroupTemplate: function getGroupTemplate(group_id) {
            var conditions = [];
...
<dl id="' + group_id + '" class="rules-group-container" ' + (this.settings.sortable ? 'draggable="true"' : '') + '> \
  <dt class="rules-group-header"> \
    ...
    <div class="btn-group"> \
      <button type="button" class="btn btn-xs btn-success" data-add="rule"><i class="fa fa-plus" aria-hidden="true"></i> ' + this.lang.add_rule + '</button> \
      <button type="button" class="btn btn-xs btn-success" data-add="group"><i class="fa fa-plus-square" aria-hidden="true"></i> ' + this.lang.add_group + '</button> \
      <button type="button" class="btn btn-xs btn-danger" data-delete="group"><i class="fa fa-minus" aria-hidden="true"></i> ' + this.lang.delete_group + '</button> \
    </div> \
    ' + (this.settings.sortable ? '<div class="drag-handle"><i class="fa fa-sort" aria-hidden="true"></i></div>' : '') + ' \
  </dt> \
  <dd class=rules-group-body> \
    <ul class=rules-list></ul> \
          ...
          getRuleTemplate: function getRuleTemplate(rule_id) {
            return RuleTemplate({
              rule_id: rule_id,
     ...
     * @returns {*}
     */
    getQuery: function getQuery(rules) {
     ...
     * @param qtree
     */
    getRulesFromQTree: function getRulesFromQTree(qtree) {
      var rules = this.rulesTranslator.convertQTreeToRules(qtree); // we need to check/modify the rules to fit the constraints
      // that we are using
     ...
     */
    updateQueryBuilder: function updateQueryBuilder(query) {
      if (!this.qtreeGetter) {
        throw new Error('You must provide qtreeGetter object for this to work');
      }

      var self = this; // first parse the query string into qtree

      return this.qtreeGetter.getQTree(query).done(function (qtree) {
        // insert the original query string
        qtree.originalQuery = query; // translate qtree into 'rules'

        var rules = self.getRulesFromQTree(qtree); // and update the UI builder with them
     ...
     * @param UIRules
     */
    checkRulesConstraints: function checkRulesConstraints(UIRules) {
      this._checkRulesConstraints(UIRules);

      return UIRules;
    },
    _checkRulesConstraints: function _checkRulesConstraints(uiRules) {
      /* if (uiRules.field) {
         var m;
     ...
     *
     */
    isDirty: function isDirty(queryToCompare) {
      if (!this.$el.data('queryBuilder')) return false;

      try {
        var formQ = this.getQuery();
        if (queryToCompare && formQ && formQ != queryToCompare) return true;
        if (formQ == '') return false;
    ...
    setQTreeGetter: function setQTreeGetter(getter) {
     ...
     * @param delay
     */
    attachHeartBeat: function attachHeartBeat(callback, delay) {
      if (!delay) {
        delay = 100;
      }
  ...
  QueryBuilder.buildQTreeGetter = function (beehive) {
    var getter = function getter(beehive) {
    ...
    _.extend(getter.prototype, {
       ...
       * @returns {Deferred}
       */
      getQTree: function getQTree(query) {
        this.promise = $.Deferred();
        this.getPubSub().publish(this.getPubSub().GET_QTREE, new ApiQuery({
          q: query
        }));
        return this.promise;
      },
       ...
       * @param apiResponse
       */
      getResponse: function getResponse(apiResponse) {
