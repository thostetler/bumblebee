define(['marionette', 'js/components/api_query', 'js/widgets/base/base_widget', 'js/widgets/library_individual/views/library_header', 'js/widgets/library_individual/views/manage_permissions', 'hbs!js/widgets/library_individual/templates/layout-container', 'hbs!js/widgets/library_individual/templates/loading-library'], function (Marionette, ApiQuery, BaseWidget, HeaderView, AdminView, ContainerTemplate, LoadingTemplate) {
  ...
  var Library = BaseWidget.extend({
    initialize: function initialize(options) {
      ...
      this.model = new StateModel(); // need to make sure view is rendered at least 1x before it shows a subview
    ...
    activate: function activate(beehive) {
      ...
      pubsub.subscribe(pubsub.LIBRARY_CHANGE, this.onLibraryChange); // now that beehive is present, attach event handlers to header view

      this.headerView.on('all', this.handleHeaderEvents, this);
    },
    onLibraryChange: function onLibraryChange(collectionJSON, info) {
    ...
    updateHeader: function updateHeader() {
      ...
      this.getBeeHive().getObject('LibraryController').getLibraryMetadata(this.model.get('id'), !this.model.get('publicView')).done(done);
    },
    updateSubView: function updateSubView() {
      ...
      that.view.main.show(new LoadingView()); // create header
    ...
    handleAdminEvents: function handleAdminEvents(event, arg1, arg2) {
      var that = this;
      ...
      switch (event) {
        case 'update-public-status':
          var data = {
            public: arg1
              ...
              id = this.model.get('id');
          libController.updateLibraryMetadata(id, data).done(function (response, status) {
        ...
        case 'confirm-transfer-ownership':
          libController.transferOwnership(this.model.get('id'), arg1).done(arg2.done).fail(arg2.fail);
          break;

        case 'reset-and-navigate':
          var ps = this.getPubSub(); // invalidates the cache of entries so the list gets refreshed
    ...
    handleHeaderEvents: function handleHeaderEvents(event, arg1, arg2) {
      var that = this,
          id = this.model.get('id'),
