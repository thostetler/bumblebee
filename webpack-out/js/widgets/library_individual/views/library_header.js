define(['marionette', 'js/widgets/library_individual/templates/library-header.html', 'moment', 'bootstrap'], function (Marionette, LibraryHeaderTemplate, moment, Bootstrap) {
  var LibraryTitleModel = Backbone.Model.extend({
    initialize: function initialize(vals, options) {
      options = options || {};
  ...
  var LibraryTitleView = Marionette.ItemView.extend({
    ...
    submitEdit: function submitEdit(e) {
      ...
      var data = {}; // If there are no changes or empty string, don't update
    ...
    triggerSubviewNavigate: function triggerSubviewNavigate(e) {
      var $current = $(e.currentTarget);
      var subView = $current.data('tab');
      var tabToShow;
      var additional; // dropdowns have multiple sub-options

      if (subView.indexOf('-') > -1) {
        tabToShow = subView.split('-')[0]; // this tells other widget which view to show
    ...
    triggerDeleteLibrary: function triggerDeleteLibrary() {
      this.trigger('delete-library');
    },
    triggerStartSearch: function triggerStartSearch() {
      // hide icons
      this.$('.bigquery-export i').css('display', 'none'); // show loader
