...
define(['underscore', 'jquery', 'backbone', 'marionette', 'bootstrap', 'js/components/api_query', 'js/components/api_request', 'js/components/api_response', 'js/components/api_feedback', 'js/components/alerts', 'js/widgets/base/base_widget', 'hbs!js/widgets/green_button/templates/widget-view', 'hbs!js/widgets/green_button/templates/item-view', 'hbs!js/widgets/green_button/templates/empty-view'], function (_, $, Backbone, Marionette, Bootstrap, ApiQuery, ApiRequest, ApiResponse, ApiFeedback, Alerts, BaseWidget, WidgetTemplate, ItemTemplate, EmptyTemplate) {
  ...
  var ItemView = Marionette.ItemView.extend({
    tagName: 'div',
    // this view will create <div class="s-hello">....</div> html node
    className: 's-service',
  ...
  var WidgetView = Marionette.CompositeView.extend({
    ...
    childView: ItemView,
    emptyView: EmptyView,
    attachHtml: function attachHtml(collectionView, itemView, idx) {
    ...
    onChildviewUserAction: function onChildviewUserAction(childView, action, model) {
  ...
  var Controller = BaseWidget.extend({
    ...
    changeState: function changeState() {// hack, to avoid the spinning wheel
    ...
    onRequest: function onRequest(apiQuery) {
    ...
    onResponse: function onResponse(apiResponse) {
    ...
    refreshStore: function refreshStore(data) {
     ...
     * @param data
     */
    updateCollections: function updateCollections(data) {
      var coll = this._transform(data);

      this.model.set('cols', coll.cols, {
        silent: true
      });
      this.collection.reset(coll.models);
    },
    showAlert: function showAlert(msg, modal, events) {
      modal = modal || false;
    ...
    onUserConfirmation: function onUserConfirmation(data) {
    ...
    onUserAction: function onUserAction(action, model, events) {
      ...
      switch (action) {
        ...
        case 'restart':
          this.showAlert('Are you sure you want to restart ' + env + ' (application: ' + app + ')? <a href="#">Yes!</a>', true, {
            'click a': {
              signal: 'confirmation',
        ...
        case 'revert':
          if (action_arg) {
            this.showAlert('Are you sure you want to revert ' + env + ' (' + app + '). To version: ' + action_arg + '? <a href="#">Yes!</a>', true, {
              'click a': {
                signal: 'confirmation',
                action: Alerts.ACTION.CALL_PUBSUB,
                arguments: {
    ...
    /*
       ...
       */
    _transform: function _transform(data) {
      var envs = {};
      var apps = {};
    ...
    /*
       ...
       */
    _sortApps: function _sortApps(appNames) {
      return appNames.sort(); // TODO
    },
    _sortEnvs: function _sortEnvs(envNames) {
    ...
    _createAppModel: function _createAppModel(appName) {
      var data = {
        id: appName,
    ...
    _shortenVersion: function _shortenVersion(label) {
    ...
    _createEnvModel: function _createEnvModel(appName, envName, data) {
      data = _.extend({
        id: appName + ':' + envName,
    ...
    _createEmptyEnvModel: function _createEmptyEnvModel(appName, envName) {
