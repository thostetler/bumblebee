  ...
  var RecommenderWidget = BaseWidget.extend({
    ...
    processResponse: function processResponse(data) {
      ...
      if (data.recommendations) {
        this.collection.reset(data.recommendations); // right now this is being ignored by the toc widget

        this.trigger('page-manager-event', 'widget-ready', {
          isActive: true
        });
      } else if (data.Error && data.Error === 'Unable to get results!') {// do nothing, user doesn't need to know
      }
    }
  });
