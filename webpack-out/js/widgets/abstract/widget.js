  ...
  var AbstractModel = Backbone.Model.extend({
    ...
    parse: function parse(doc) {
      ...
      var getDOIUrl = function getDOIUrl(doi) {
        return LinkGeneratorMixin.createUrlByType(doc.bibcode, 'doi', doi);
      ...
      var defaultList = _.range(numAuthors).map(function () {
        return '-';
      });
      ...
      if (doc.aff.length) {
        ...
        doc.authorAff = _.zip(doc.author, doc.aff, doc.orcid_pub ? doc.orcid_pub : defaultList, doc.orcid_user ? doc.orcid_user : defaultList, doc.orcid_other ? doc.orcid_other : defaultList);
      } else if (doc.author) {
        doc.hasAffiliation = false;
        doc.authorAff = _.zip(doc.author, _.range(doc.author.length), doc.orcid_pub ? doc.orcid_pub : defaultList, doc.orcid_user ? doc.orcid_user : defaultList, doc.orcid_other ? doc.orcid_other : defaultList);
      }
    ...
    copyBibcode: function copyBibcode() {
      if (!this.bibcodeClipboard) {
        ...
        this.bibcodeClipboard.on('success', function () {
          $('#abs-bibcode-copy-msg').show().fadeOut(1000);
        });
      }
    },
