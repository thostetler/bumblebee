define(['marionette', 'js/widgets/base/base_widget', 'js/components/api_request', 'js/components/api_response', 'js/components/api_query', 'js/widgets/graphics/templates/grid.html', 'js/widgets/graphics/templates/sidebar.html', 'js/components/api_targets', 'analytics'], function (Marionette, BaseWidget, ApiRequest, ApiResponse, ApiQuery, gridTemplate, sidebarTemplate, ApiTargets, analytics) {
  ...
  var GridView = Marionette.ItemView.extend({
    ...
    fireAnalyticsEvent: function fireAnalyticsEvent(ev) {
  ...
  var SidebarView = Marionette.ItemView.extend({
    ...
    serializeData: function serializeData() {
      ...
      if (graphics) {
        return {
          sampleGraphic: graphics[_.keys(graphics)[0]].thumbnail
        };
      }

      return {
        sampleGraphic: undefined
      };
    }
  });
  var GraphicsWidget = BaseWidget.extend({
    initialize: function initialize(options) {
      ...
      this.view = options.sidebar === true ? new SidebarView({
        model: this.model
      }) : new GridView({
        model: this.model
      });
    ...
    loadBibcodeData: function loadBibcodeData(bibcode) {
      ...
      this.getPubSub().publish(this.getPubSub().DELIVERING_REQUEST, request); // now ask for the title if it's the main widget
    ...
    getResponseDeferred: function getResponseDeferred() {
    ...
    processResponse: function processResponse(response) {
      if (!(response instanceof ApiResponse)) {
        var responseDef = this.getResponseDeferred(); // it's from the graphics service
        ...
        _.each(response.get('figures'), function (dict) {
          graphics[dict.figure_label] = dict.images[0]; // check for interactive graphics
    ...
    triggerShowGrid: function triggerShowGrid() {
