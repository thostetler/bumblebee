 ...
 */
define(['underscore', 'jquery', 'backbone', 'marionette', 'js/components/api_request', 'js/components/api_query', 'js/components/pubsub_events', 'js/widgets/api_request/templates/widget-view.html', 'js/widgets/api_request/templates/item-view.html', 'js/mixins/dependon'], function (_, $, Backbone, Marionette, ApiRequest, ApiQuery, PubSubEvents, WidgetTemplate, ItemTemplate, Dependon) {
  // Model
  ...
  var WidgetView = Marionette.CompositeView.extend({
    ...
    loadApiRequest: function loadApiRequest(ev) {
    ...
    runApiRequest: function runApiRequest(ev) {
    ...
    updateResultsText: function updateResultsText(data) {
      this.$el.find('#api-request-result').text(data);
    }
  });
  var WidgetController = Marionette.Controller.extend({
    initialize: function initialize(apiRequest) {
    ...
    onLoad: function onLoad(apiRequest) {
      if (this.collection) {
     ...
     * @returns {*|Array}
     */
    getData: function getData(apiRequest) {
      if (!apiRequest) {
     ...
     * @param model
     */
    onRun: function onRun(apiRequest) {
      if (this.hasPubSub()) {
