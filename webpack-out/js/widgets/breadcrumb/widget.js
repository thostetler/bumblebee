...
define(['underscore', 'jquery', 'backbone', 'marionette', 'js/components/api_query', 'js/components/pubsub_events', 'js/widgets/breadcrumb/templates/widget-view.html', 'js/widgets/breadcrumb/templates/item-view.html', 'js/mixins/dependon'], function (_, $, Backbone, Marionette, ApiQuery, PubSubEvents, WidgetTemplate, ItemTemplate, Dependon) {
  ...
  var WidgetController = Marionette.Controller.extend({
    ...
    activate: function activate(beehive) {
      ...
      pubsub.subscribe(pubsub.DELIVERING_REQUEST, this.onRequest); // pubsub.subscribe('all', this.onAllPubSub);
    ...
    onNewQuery: function onNewQuery(apiQuery, key) {
      console.log('START_SEARCH', apiQuery.url(), key);
      this.listening = true;
    },
    onRequest: function onRequest(apiRequest, key) {
      if (!this.listening) return;
      var q = apiRequest.get('query');

      if (q) {
        console.log('NEW REQUEST', q); // remember who initiated the new-query
     ...
     * that back views
     */
    onAllInternalEvents: function onAllInternalEvents() {
