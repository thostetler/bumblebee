  ...
  var PreferencesWidget = BaseWidget.extend({
    ...
    handleViewEvents: function handleViewEvents(event, arg1, arg2) {
      ...
      if (event === 'change:link_server') {
        ...
        this.getBeeHive().getObject('User').setUserData(arg1).done(_.bind(subView.onSuccess, subView)).fail(_.bind(subView.onError, subView));
      } else if (event === 'change:exportSettings') {
        var _subView = this.view.content.currentView;
        this.getBeeHive().getObject('User').setUserData(arg1).done(_.bind(_subView.onSuccess, _subView)).fail(_.bind(_subView.onError, _subView));
      }
    },
