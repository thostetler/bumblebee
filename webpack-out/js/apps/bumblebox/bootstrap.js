 ...
 */
define(['underscore', 'jquery', 'backbone', 'js/components/api_query', 'js/components/api_request', 'js/components/pubsub_events'], function (_, $, Backbone, ApiQuery, ApiRequest, PubSubEvents) {
  var Mixin = {
    /**
     ...
     */
    bootstrap: function bootstrap(conf) {
      conf = conf || {};
      var defer = $.Deferred(); // load configuration from remote endpoints
     ...
     * @returns {*}
     */
    onBootstrap: function onBootstrap(app_config, dynamic_config) {
      ...
      var enhanceConfig = function enhanceConfig(targetConfig, dynamic_config) {
     ...
     * @param loadedConfig
     */
    configure: function configure(loadedConfig) {
      ...
      this.getBeeHive().getService('Api').setVals({
        access_token: "Bearer ".concat(conf.access_token),
        refresh_token: conf.refresh_token,
        // will probably be null....
        expires_in: conf.expires_in,
        clientVersion: null,
        // to avoid sending the headers (for now)
    ...
    start: function start(Router) {
      ...
      masterPageManager.assemble(app); // attach the master page to the body

      $(conf.targetElement || 'div#body-template-container').empty().append(masterPageManager.view.el); // kick off routing
