...
define(['module'], function (module) {
  require(['router', 'js/components/application', 'js/apps/bumblebox/bootstrap', 'dynamic_config', 'underscore'], function (Router, Application, AppBootstrap, DynamicConfig, Es5Shim, _) {
    Application.prototype.shim(); // load the urls of dynamic config from the script element

    _.each(document.getElementsByTagName('script'), function (scriptNode) {
      if (scriptNode.hasAttribute('data-main') && (scriptNode.getAttribute('data-main') || '').indexOf('embed.config') > -1 && !scriptNode.hasAttribute('data-bbb-inuse')) {
    ...
    app.bootstrap(DynamicConfig).done(function (loadedConfig) {
      var loadConfig = app.onBootstrap(module.config(), loadedConfig); // load the objects/widgets/modules

      app.loadModules(loadConfig).done(function () {
