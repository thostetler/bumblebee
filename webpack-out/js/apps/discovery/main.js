...
define(['config/discovery.config', 'module'], function (config, module) {
  ...
  require(['router', 'js/components/application', 'js/mixins/discovery_bootstrap', 'js/mixins/api_access', 'js/components/api_feedback', 'analytics'], function (Router, Application, DiscoveryBootstrap, ApiAccess, ApiFeedback, analytics) {
    ...
    var startApp = function startApp() {
      updateProgress(80, 'App Loaded'); // this will activate all loaded modules
        ...
        app.start(Router).done(function () {
          ...
          $('body').on('click', '#results-actions-toggle', function () {
            ...
            if ($('#query-info-container').hasClass('show')) {
              $('#results-actions-toggle').html('<i class="fa fa-times" title="close actions" aria-hidden="true"></i> Actions');
            } else {
              $('#results-actions-toggle').html('<i class="fa fa-book" title="open actions" aria-hidden="true"></i> Actions');
            }
          ...
          if (window.__PAGE_LOAD_TIMESTAMP) {
            ...
            analytics('send', 'event', 'uitheme', // category
            action, label);
