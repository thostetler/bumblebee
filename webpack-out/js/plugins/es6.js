  ...
  return {
    ...
    load: function load(name, req, onload, config) {
      ...
      try {
        ...
        babelOptions = JSON.parse(JSON.stringify(config.babel)); // clone callbacks from babel config manually
      ...
      var pluginOptions = config.es6 || {},
          ...
          url = req.toUrl(sourceFileName); // Do not load if it is an empty: url
    ...
    write: function write(pluginName, moduleName, _write) {
      if (moduleName in buildMap) {
        _write.asModule(pluginName + '!' + moduleName, buildMap[moduleName]);
      }
    }
  };
});
